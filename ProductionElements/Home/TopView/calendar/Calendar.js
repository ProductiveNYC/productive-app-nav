...
export default class Calendar extends PureComponent {
  ...
  getVisibleMonthAndYear = (): ?string => {
    ...
    return `${visibleMonths[0]} ${visibleYears[0]}`;

    /* IF WE WANTED TO SHOW MONTH - NEXT MONTH */
  ...
  scrollToCurrentDay = () => {
    ...
    // let finalX = Math.floor(scrollToX / screenWidth) * screenWidth
  ...
  onRenderDay = (index: number, width: number) => {
    ...
    this.setState(prevState => ({
      ...
      dayWidths: {
        ...
        [index]: width + (2 * (2 * screenWidth / 9) / 7 / 2),
      },
    }), () => {
      if (allDatesHaveRendered) {
